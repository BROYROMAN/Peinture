...
function actionAccueil($twig, $db){
   $tableau = new Tableau($db);
    $liste = $tableau->select();
     $livreor = new Livreor($db);
    $liste2 = $livreor->select();
      ...
      $form[] = null;
                  ...
                  $nb = $livreor->insert($inputPseudo,$inputEmail,$inputMessage,$inputNote,$date,$etat);
                  if ($nb!=1){
    ...
    echo $twig->render('index.html.twig', array('listetab'=>$liste,'listeavis'=>$liste2));
}


function actionLivreOr($twig, $db){
    $livreor = new Livreor($db);
    $liste = $livreor->selectAvis();    
    $nb = count($liste);
    echo $twig->render('livreor.html.twig', array('listelo'=>$liste,'nb'=>$nb));
}
