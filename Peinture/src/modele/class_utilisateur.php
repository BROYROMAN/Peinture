...
class Utilisateur{
    ...
    public function __construct($db){
        $this->db = $db;
        $this->insert = $db->prepare("insert into utilisateur(email, mdp, nom, prenom,cp,ville, idRole,photo) values (:email, :mdp, :nom, :prenom,:cp,:ville, :role,:photo)");    
        $this->connect = $db->prepare("select email, idRole, mdp from utilisateur where email=:email");
        $this->select = $db->prepare("select email, idRole, nom, prenom,ville,cp,photo, r.libelle as libellerole,u.id as idu from utilisateur u, role r where u.idRole = r.id order by nom");
    ...
    public function insert($email, $mdp, $role, $nom, $prenom,$cp,$ville,$photo){
        $r = true;
        $this->insert->execute(array(':email'=>$email, ':mdp'=>$mdp, ':role'=>$role, ':nom'=>$nom, ':prenom'=>$prenom,':cp'=>$cp, ':ville'=>$ville,':photo'=>$photo));
    ...
    public function select(){
     ...
     public function PDF(){
        $this->PDF->execute();
        if ($this->PDF->errorCode()!=0){
             print_r($this->PDF->errorInfo());  
        }
        return $this->PDF->fetchAll();
    }
